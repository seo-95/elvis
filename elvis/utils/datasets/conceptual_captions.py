...
class ConceptualCaptions(Dataset):
    def __init__(self,
        ...
        self.name    = 'GCC'
        self.info    = '{} {}'.format(raw_data['name'], raw_data['split'])
        self.split   = raw_data['split']
        self.dataset = raw_data['dataset']
...
def build_gcc_dataset(cfg, **kwargs):
    trainset = ConceptualCaptions(root=cfg.ROOT,
                                  annotations_file=cfg.TRAIN[0],
                                  images_dir=cfg.IMAGES_DIR,
                                  worker_fn=kwargs['worker_fn'],
                                )
    devset = ConceptualCaptions(root=cfg.ROOT,
